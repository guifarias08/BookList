...
class BookController extends Controller
{
    public function index(Request $request)
    {
        ...
        switch ($request->input('sort')) {
            ...
            case 'oldest':
                ...
                break;
            case 'rating':
                $query->orderByDesc('rating')->orderBy('title');
                break;
            case 'recent':
                $query->latest();
                break;
        ...
        $totalAuthors = Book::whereNotNull('author')->where('author', '!=', '')->distinct()->count('author');
        $totalGenres = Book::whereNotNull('genre')->where('genre', '!=', '')->distinct()->count('genre');
        ...
        $averageRating = Book::whereNotNull('rating')->avg('rating');
    ...
    public function favorite(Book $book)
    {
        ...
        return back()->with(
            'success',
            $book->favorite ? 'Livro adicionado aos favoritos!' : 'Livro removido dos favoritos.'
        );
    }
    ...
    private function validationRules(): array
    {
        return [
            ...
            'status' => ['required', 'in:want_to_read,reading,read,paused,abandoned'],
    ...
    private function validationMessages(): array
    {
        return [
            ...
            'cover.mimes' => 'A capa deve ser JPG, JPEG, PNG ou WEBP.',
            'cover.max' => 'A imagem deve possuir no máximo 2 MB.',
            'pages.min' => 'O livro precisa possuir pelo menos uma página.',
        ];
    }
        ...
        if (
            ...
            abort(
                redirect()
                    ...
                    ->withInput()
                    ->withErrors([
                        'current_page' => 'A página atual não pode ser maior que o total de páginas.',
                    ])
            );
        }
    }
